...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_out, enter, esc, sequence, sequence_inv, seq_len, teach_rate):
        ...
        #print("sequence inv sorted", sequence_inv)
        ...
        predictions = torch.zeros((batch_size, seq_shape-1, 2)).to(device)  # (b,max_len(2~t),e_size)
        predictions_inv = torch.zeros((batch_size, seq_shape-1, 2)).to(device)  # (b,max_len,e_size)
        predictions_assemble = torch.zeros((batch_size,seq_shape,2)).to(device)
        ...
        # TODO: seq shape is not max lenght, doing lstm cell runs with 0 0
        # -1 for tesing
        max_seq = torch.max(seq_len)
        ...
        for t in range(max_seq):
            ...
            # why is it a tensor of true or false?
            # tensor to list to get rid of true false
            #print("seq_len", seq_len, type(seq_len))
            # seq_len from here to seq_len_l
            seq_len_l = seq_len.tolist()
            # TODO: batch size was weird
            batch_size_t = sum([l > t for l in seq_len_l]) # manual batch size for testing
            #print("t", t)
            #print("batchsizet", batch_size_t)
            # error when batch size is 0
            # but we have to stop at batch size 0
            if batch_size_t == 0:
            ...
            # added dim=X
            # weight = F.softmax(self.attention(h_a[:batch_size_t])
            weight = F.softmax(self.attention(h_a[:batch_size_t])) # weight for each input pixels
            weight_inv = F.softmax(self.attention(h_b[:batch_size_t]))
            #weight_inv = F.softmax(self.attention(h_b[:batch_size_t]), dim=0) # (batch_size_t,n_pixels)
            # what is the t in encoder_out doing?
            if use_teacher_forcing:
                print("teacher_forcing 1");
                h, c = self.decoder(
                    torch.cat([sequence[:batch_size_t,t+1,:],encoder_out[:batch_size_t,:,0] * weight],dim=1),
                    (h_a[:batch_size_t,:], c_a[:batch_size_t,:]))  # (batch_size_t, decoder_dim)
                h_inv, c_inv = self.decoder_inv(
                    torch.cat([sequence_inv[:batch_size_t,t+1,:],encoder_out[:batch_size_t,:,0] * weight_inv],dim=1),
                    (h_b[:batch_size_t,:], c_b[:batch_size_t,:]))
            else:
                # TODO: schau was preds genau macht, nimmt die predictions bis jetzt
                print("teacher_forcing 0");
                h, c = self.decoder(
                    torch.cat([self.position_embedding(preds[:batch_size_t,:]),encoder_out[:batch_size_t,:,0] * weight],dim=1),
                    (h_a[:batch_size_t,:], c_a[:batch_size_t,:]))  # (batch_size_t, decoder_dim)
                h_inv, c_inv = self.decoder_inv(
                    torch.cat([self.position_embedding(preds_inv[:batch_size_t,:]),encoder_out[:batch_size_t,:,0] * weight_inv],dim=1),
                    (h_b[:batch_size_t,:], c_b[:batch_size_t,:]))
            h = self.trans_h(h)
            c = self.trans_c(c)
            ...
            # TODO misuse of seq_shape again
            for b in range(batch_size_t):
                predictions_assemble[b, seq_len[b]-1-t,:] = preds_inv[b]
            ...
            #print("working on predictions", predictions)
            #print("wokring on pred inv", predictions_inv)
            #print("pred assem?", predictions_assemble)
        ...
        #print("chgeck pred ass after add", predictions_assemble)

        predictions_assemble[:, 1:-1, :] /= 2
        #print("shouldnÄt be that here?", predictions_assemble)
        # TODO: not predic assemble?
        #predictions[:,1:-1,:] /= 2
