...
if __name__ == '__main__':
    #map_whole = cv.imread('D:/BA/Two-Stage-Gan-in-trajectory-generation/pre_process/map_generation/output_merge.png')
    #map_grids = grid(map_whole)  # map_grids[point] point = (x,y)
    #for i in range (32):
    #    for j in range(32):
    #        num = (32 * i) + j
    #        enter_pointld, exit_pointld = get_random_enter_exit_point(
    #            map_grids['%d_%d' % (i, j)], 'left', 'down', downsample=0.2)
    #        # enter_pointlu, exit_pointlu = get_random_enter_exit_point(
    #        #    map_grids['%d_%d' % (i, j)], 'left', 'up', downsample=0.2)
    #        # np.save('./test/%s.npy' % ('0' * (4 - len(str(num))) + str(num)), np.array([[enter_pointlu[0], exit_pointlu[0]],[enter_pointld[0], exit_pointld[0]]]))
    #        np.save('./test/%s.npy' % ('0' * (4 - len(str(num))) + str(num)), np.array([[enter_pointld[0], exit_pointld[0]]]))
    map_whole = cv.imread('P:/BA/TSG/TSG/pre_process/map_generation/output_merge.png')
    map_grids = grid(map_whole)  # map_grids[point] point = (x,y)
    output_path = 'P:/BA/TSG/TSG/TSG/map_dict/'
    # print(map_grids['1_2'])
    cv.imencode('.png', map_grids['0_0'])[1].tofile(output_path + grid_name + '.png')
    for i in range(32):
        for j in range(32):
            grid_name = '%s_%s' % (i, j)
            cv.imencode('.png', map_grids[grid_name])[1].tofile(output_path  + grid_name+'.png')
