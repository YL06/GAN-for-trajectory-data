...
parser.add_argument('--dataroot', default='./grid_data_forfirst/data_1000/', help='path to dataset')
...
parser.add_argument('--outf', default='./output/data_1000/', help='folder to output images and model checkpoints')
...
def weights_init(m):
    ...
    if classname.find('Conv') != -1:
        ...
        m.weight.data.normal_(1.0, 0.02) #YJ_0418_WeightInit (1.0 -> 0.0)
        m.bias.data.fill_(0)
    ...
    for epoch in range(opt.start_iter,opt.niter):
        for i, (real_imgs) in enumerate(dataloader):
            ...
            if i % 100 == 0:
                ...
                # normalize true: shift the image to the range (0, 1)
                # f'{opt.outf}/real_samples.png' to manual file path cause of errno 13 permission denied
                #vutils.save_image((real_imgs/2+0.5)*795,
            #                    'P:/BA/TSG/TSG/First_stage_gan/output/real_samples.png',
                #                normalize=True)
                ...
                vutils.save_image((netG(noise).detach()/2+0.5)*795,
                                f'{opt.outf}/fake_samples_epoch_{epoch}_{i}.png',
                                normalize=True)
                vutils.save_image((netG(noise).detach()/2+0.5)*795,f'{opt.outf}/fake_samples_epoch_{epoch}_{i}_false.png',normalize=False)
